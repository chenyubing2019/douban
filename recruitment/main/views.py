...
from .models import PersonInfo, CompanyInfo, PostJob
...
def index(request):
    ...
    # # 游戏
    # game_total = "游戏"
    # game_list = ["技术开发", "产品策划", "设计", "运营/推广", "其他"]
    ...
    try:
        user_info = request.session.get("person")
        user_info["name"] = UserInfo.objects.get(phone=user_info["phone"]).name
        try:
            company_info = models.PostJob.objects.all()
        except:
            company_info = None
        if user_info:
            return render(request, "main/index.html", {"user_info": user_info, "company_info": company_info})
        else:
...
def personinfo(request):
    if request.method == "GET":
        phone = request.session.get("person")["phone"]      # 得到登录的手机号

        user_info = UserInfo.objects.get(phone=phone)       # 获取账号信息
        try:
            person_info = PersonInfo.objects.get(phone=phone)   # 获取基本信息
        except:
            person_info = None
        return render(request, "main/personinfo.html", {"user_info": user_info, "person_info": person_info})
    if request.method == "POST":
        ...
        # 保存个人资料
        try:
            if PersonInfo.objects.filter(phone=phone).exists():
                # result = "修改失败"
                person_info = PersonInfo.objects.filter(phone=phone)
                person_info.update(name=name, age=age, sex=sex, school=school, prof=prof, education=education)
            else:
                # result = "创建失败："
                PersonInfo.objects.create(phone=phone, name=name, age=age, sex=sex, school=school, prof=prof, education=education)
            return redirect("../")
        except:
            return HttpResponse("result")
...
def companyinfo(request):
    if request.method == "GET":
        ...
        user_info = UserInfo.objects.get(phone=phone)       # 获取账号信息
        try:
            company_info = CompanyInfo.objects.get(phone=phone)   # 获取基本信息
        except:
            print("exception")
            company_info = None
        return render(request, "main/companyinfo.html", {"user_info": user_info, "company_info": company_info})
    ...
    if request.method == "POST":
        ...
        reg_fund = request.POST.get("reg_fund")
        staff_scale = request.POST.get("staff_scale")

        # 保存公司资料
        try:
            if CompanyInfo.objects.filter(phone=phone).exists():
                result = "修改失败"
                company_info = CompanyInfo.objects.filter(phone=phone)
                company_info.update(name=name, address=address, business_id=business_id, group=group, legal_person=legal_person, reg_fund=reg_fund, staff_scale=staff_scale)
            else:
                result = "创建失败："
                CompanyInfo.objects.create(phone=phone, name=name, address=address, business_id=business_id, group=group, legal_person=legal_person, reg_fund=reg_fund, staff_scale=staff_scale)
            return redirect("../")
...
def postjob(request):
    if request.method == "GET":
        return render(request, "main/postjob.html")
    elif request.method == "POST":
        name = request.POST.get("name")                 # 公司名
        recruitment = request.POST.get("recruitment")   # 招募的职业
        pay = request.POST.get("pay")                   # 薪资待遇
        area = request.POST.get("area")                 # 工作城市
        experience = request.POST.get("experience")     # 工作经验
        education = request.POST.get("education")       # 学历
        post = request.POST.get("post")                 # 职位要求（全职，兼职，实习）
        attract = request.POST.get("attract")           # 职业诱惑
        describe = request.POST.get("describe")         # 职位描述
        workplace = request.POST.get("workplace")       # 工作地点
        # time = request.POST.get("time")               # 发布时间
        phone = request.session.get("person")["phone"]
        # print(name, recruitment,  pay, area, experience, education, post, attract, describe, workplace)
        p = PostJob(phone=phone, name=name, recruitment=recruitment,  pay=pay, area=area, experience=experience, education=education, post=post, attract=attract, describe=describe, workplace=workplace)
        p.save()
        return HttpResponse("录入成功,<a href='../'>返回首页</a>")

# 公司招聘详细信息
def job_detail(request, id):
    postjob = models.PostJob.objects.get(pk=id)
    return render(request, "main/jobdetail.html", {"postjob": postjob})
